              ...
              <div class="row">
                ...
                <div class="col-md-3 col-sm-6 col-xs-6">
                  <select name='os'>
                    ...
                    <option value='ios' {{ Request::input('os') == 'ios' ? 'selected' : '' }}>SUPER</option>
    ...
    <section class="section-products">
      <div class="section-header">
        <div class="section-header-left">
          <h2 class="section-title">GUNDAME</h2>
        </div>
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-12 col-sm-12 col-xs-12 animate">
                        <div class="product-details">
                          <p><strong><i class="fas fa-tv"></i> Thuộc Series: </strong>{{ $product->monitor }}</p>
                          <p><strong><i class="fas fa-camera-retro"></i> Mặt trước: </strong>{{ $product->front_camera }}</p>
                          <p><strong><i class="fas fa-camera-retro"></i> Mặt sau: </strong>{{ $product->rear_camera }}</p>
                          <p><strong><i class="fas fa-microchip"></i> Dạng chuyển: </strong>{{ $product->CPU }}</p>
                          <p><strong><i class="fas fa-microchip"></i>Dạng phát triển: </strong>{{ $product->GPU }}</p>
                          <p><strong><i class="fas fa-hdd"></i> Biên độ cử động: </strong>{{ $product->RAM }}G</p>
                          <p><strong><i class="fas fa-hdd"></i> Cân nặng: </strong>{{ $product->ROM }}G</p>
                          @if(Str::is('*Android*', $product->OS_version))
                            <p><strong><i class="fab fa-android"></i> HĐH:Super </strong>{{ $product->OS_version }}</p>
                          @elseif(Str::is('*IOS*', $product->OS_version))
                            <p><strong><i class="fab fa-apple"></i> HĐH:BanDai </strong>{{ $product->OS_version }}</p>
                          @elseif(Str::is('*Windows*', $product->OS_version))
                            <p><strong><i class="fab fa-windows"></i> HĐH:Japan </strong>{{ $product->OS_version }}</p>
                          @endif
                          <p><strong><i class="fas fa-battery-full"></i> Cơ năng hoạt động: </strong>{{ $product->pin }}</p>
                        </div>
                      </div>
                    </div>
                  </a>
